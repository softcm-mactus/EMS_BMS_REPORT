...
namespace EmsBMSReports
{
    ...
    public partial class DlgReportsConfiguration
    {
        public DlgReportsConfiguration()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                using (var oConnection = new OdbcConnection(g_sConString))
                {
                    ...
                    while (oReader.Read())
                    {
                        ...
                        if (nReportType == MactusReportLib.MactusReportLib.ReportType.DataReport)
                        {
                            nRow = oGrid.Rows.Add();
        ...
        private void bAddNewReport_Click(object sender, EventArgs e)
        {
            ...
            if (oDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                using (var oConnection = new OdbcConnection(g_sConString))
                {
                    ...
                    oCmd.Parameters.Add("@1", OdbcType.Int).Value = MactusReportLib.MactusReportLib.ReportType.DataReport; // reporttype
                    ...
                    //if (g_dbType == DBType.EBODB) // datatablename
                    //{
                    ...
                    if (nReportID > 0)
                    {
                        AddDateTimeColumnToReport(nReportID, g_dbType, MactusReportLib.MactusReportLib.ReportType.DataReport);
                    }
                    oConnection.Close();
