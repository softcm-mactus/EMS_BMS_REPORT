...
namespace EmsBMSReports
{
    ...
    public partial class DlgExcursionConfiguration
    {
        public DlgExcursionConfiguration()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                using (var oConnection = new OdbcConnection(g_sConString))
                {
                    ...
                    while (oReader.Read())
                    {
                        ...
                        if (nReportType == MactusReportLib.MactusReportLib.ReportType.DataTrendAlarmReport)
                        {
                            nRow = oGrid.Rows.Add();
        ...
        private void bAddNewReport_Click(object sender, EventArgs e)
        {
            ...
            if (oDlg.ShowDialog() == DialogResult.OK)
            {
                ...
                using (var oConnection = new OdbcConnection(g_sConString))
                {
                    ...
                    oCmd.Parameters.Add("@1", OdbcType.Int).Value = MactusReportLib.MactusReportLib.ReportType.DataTrendAlarmReport;  // reporttype
