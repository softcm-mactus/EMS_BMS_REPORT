...
Public Class EBOCombinedIndusoftReport
    ...
    Private g_nExcursionCheckCount As Int16 = 0
    Private g_bExcursion As Boolean
    Dim oAlarmTable As New DataTable
    ...
    Private Sub ReadReportColumnConfiguration(ByRef nReportID As Integer)
        ...
        Try
            ...
            While oReader.Read()
                ...
                Try
                    oCol.m_bExcursion = oReader("excursion")
                Catch ex As Exception
                    oCol.m_bLowCheck = False
                    oCol.m_fLow = 0
                End Try
                ...
                Try
                    oCol.m_fExcursionLow = oReader("excursionlowvalue")
                Catch ex As Exception
                    oCol.m_bLowCheck = False
                    oCol.m_fLow = 0
                End Try
                ...
                Try
                    oCol.m_fExcursionHigh = oReader("excursionhighvalue")
                Catch ex As Exception
                    oCol.m_bLowCheck = False
                    oCol.m_fLow = 0
                End Try
                ...
                If g_nReportType = ReportType.AlarmReport Or g_nReportType = ReportType.EventReport Or g_nReportType = ReportType.DataTrendAlarmReport Then
    ...
    Private Sub PrintLimitSetPointRows(ByRef oStartDate As Date, ByRef oEndDate As Date)
        ...
        Try
            ...
            For nIndex = 1 To g_oColList.Count - 1
                ...
                sTemp = ""
                If g_oColList(nIndex).m_bExcursion Then

                    sTemp = g_oColList(nIndex).m_fExcursionLow.ToString(g_oColList(nIndex).m_sColFormat)
            ...
            oCell = New PdfPCell(New Paragraph("Excursion High Limit", g_oBodyHeaderFont))
            oCell.BackgroundColor = New iTextSharp.text.Color(90, 190, 243)
            oCell.PaddingTop = g_nBodyPad
            oCell.PaddingBottom = g_nBodyPad
            oCell.HorizontalAlignment = Element.ALIGN_CENTER
            ...
            For nIndex = 1 To g_oColList.Count - 1
                ...
                If g_oColList(nIndex).m_bExcursion Then
                    ...
                    '    End Try
                    'Else
                    sTemp = g_oColList(nIndex).m_fExcursionHigh.ToString(g_oColList(nIndex).m_sColFormat)
                    'End If
                End If
            ...
            g_oDoc.Add(oTable)
        Catch ex As Exception
            LogError("EBOCombinedIndusoftReport.vb", "PrintLimitSetPointRows()", ex.Message)
        End Try
    ...
    Public Function GenerateExcursionReport(ByRef nReportStatusID As Long, ByVal dtFrom As Date, ByVal dtTo As Date, ByRef sOutFileName As String, ByRef nTimeInterval As Integer) As Boolean
        ...
        Try
            ...
            Using FS As New FileStream(sOutFileName, FileMode.Create, FileAccess.Write, FileShare.None)
                ...
                While oTime <= dtTo
                    'If nTimeInterval = 1 Or g_nDataAgg = DataAgg.Instance Then
                    bResult = GetColInstanceValuesForExcursionReport(oTime)
                    'Else
                    '    bResult = GetColAverageMinMaxValues(oTime, nTimeInterval)
                    ' End If
                    ...
                    If bResult = True Then
                        For nCol = 0 To g_oColList.Count - 1
                            ...
                            Try
                                ...
                                'get min value of each column And assign to m_freportmin
                                If g_oColList(nCol).m_nColType = ColType.Temperature Or g_oColList(nCol).m_nColType = ColType.Other Or g_oColList(nCol).m_nColType = ColType.Humidity Or g_oColList(nCol).m_nColType = ColType.DP Then
                                    If g_oColList(nCol).m_sValue = "****" Then
        ...
        Catch ex As Exception
            UpdateExceptionInDatabase(nReportStatusID, ex.Message)
            LogError("NewIndusoftReport.vb", "GenerateExcursionReport()", ex.Message)
        Finally
    ...
    Public Function GenerateAlarmReportUsingTrendData(ByRef nReportStatusID As Long, ByVal dtFrom As Date, ByVal dtTo As Date, ByRef sOutFileName As String, ByRef nTimeInterval As Integer) As Boolean
        ...
        Dim nTopMargin As Integer = 0
        Dim nBottomMargin As Integer = 0
        'Dim oTabAlarmHeader = New PdfPTable(6)
        Try
            ...
            Using FS As New FileStream(sOutFileName, FileMode.Create, FileAccess.Write, FileShare.None)
                ...
                For nCol = 0 To 5 - 1
                    ...
                    oHeadPdfCel = New PdfPCell(New Paragraph(sColumName(nCol).ToString, g_oBodyHeaderFont))
                    oHeadPdfCel.PaddingBottom = g_nBodyPad
                    oHeadPdfCel.BackgroundColor = New iTextSharp.text.Color(90, 190, 243)
                    oHeadPdfCel.BackgroundColor = New iTextSharp.text.Color(90, 190, 243)
                ...
                PrintEndOfDataReport(dtFrom, dtTo)
                g_oDoc.Close()
                GenerateAlarmReportUsingTrendData = True
                UpdateReportProgress(nReportStatusID, dtFrom, dtTo, dtTo)
            End Using
        ...
        Catch ex As Exception
            UpdateExceptionInDatabase(nReportStatusID, ex.Message)
            LogError("EBOCombinedIndusoftReport.vb", "GenerateAlarmReportUsingTrendData()", ex.Message)
        Finally
    ...
    Function GetColInstanceValuesForExcursionReport(ByRef oTime As Date) As Boolean
        Try
            Dim sQuery As String
            Dim oReader As OdbcDataReader
            GetColInstanceValuesForExcursionReport = False
            g_bExcursion = False
                ...
                Try
                            ...
                            ElseIf g_oColList(nCol).m_nColType = ColType.Enumtype Then
                                Try
                                    ...
                                    If g_oColList(nCol).m_bExcursion And g_oColList(nCol).m_fExcursionHigh <> 0 Then
                                        ...
                                        End If
                                    End If
                                    'If GetColInstanceValuesForExcursionReport = False Then
                                    '    g_oColList(nCol).m_sValue = "-"
                                    'End If
                                Catch ex As Exception
                    ...
                    If g_bExcursion Then
                        ...
                        If g_nExcursionCheckCount >= 3 Then
                            ...
                            GetColInstanceValuesForExcursionReport = True
                        End If
                    Else
                        g_nExcursionCheckCount = 0
                    End If
    ...
    Private Function GetColInstanceValues(ByRef oTime As Date) As Boolean
        ...
        Using oConnection As New OdbcConnection(g_sEMSDbConString)
            ...
            Try
                ...
                oReader.Close()
            Catch ex As Exception
                LogError("EBOCombinedIndsoftReport.vb", "GetColInstanceValues()", ex.Message)
            End Try
            oConnection.Close()
        End Using
        ...
        Try
            ...
            Using FS As New FileStream(sOutFileName, FileMode.Create, FileAccess.Write, FileShare.None)
                ...
                Try
                    ...
                    oTable.SetWidths(sColWidths)
                Catch ex As Exception
                    LogError("NewIndusoftReport.vb", "GenerateEventReport()", ex.Message)
                End Try
    ...
    Private Function GetNextAlarmTime(ByRef oAlmGUID As Guid, ByRef nAlmID As ULong, ByRef nNxtAlmID As ULong) As Boolean
        ...
        Catch ex As Exception
            LogError("NewIndusoftReport.vb", "GetNextAlarmTime()", ex.Message)
        End Try
    ...
    Private Function GetAlarmGroupName(ByRef nAlmGroupID As Integer, ByRef sAlmGrpName As String) As Boolean
        ...
        Catch ex As Exception
            LogError("NewIndusoftReport.vb", "GetAlarmGroupName()", ex.Message)
        End Try
    End Function
    ...
    Private Sub PrintEndOfDataReport(ByVal oStartTime As Date, ByVal oEndTime As Date)
    ...
    Private Sub PrintMinMaxRows()
        ...
        For nIndex = 1 To g_oColList.Count - 1
            ...
            sTemp = ""
            If g_oColList(nIndex).m_nColType = ColType.DP Or g_oColList(nIndex).m_nColType = ColType.Temperature Or g_oColList(nIndex).m_nColType = ColType.Humidity Or g_oColList(nIndex).m_nColType = ColType.Other Then
    ...
    Public Function IsPointDataAvailable(ByRef nPointID As Integer, ByRef oStartDate As Date) As Boolean
        ...
        sQuery = "SELECT count(*) FROM alldata WHERE timestamp > ? AND  timestamp < ?  AND Value IS NOT NULL AND externallogid =" + nPointID.ToString()
    ...
    Public Function GetPointIDValue(ByRef nPointID As Integer, ByRef oStartDate As Date, ByRef fValue As Single, ByRef bIsPoint As Boolean) As Boolean
        ...
        If bIsPoint = False Then
            sQuery = "SELECT Value FROM alldata WHERE timestamp > ? AND  timestamp < ?  AND Value IS NOT NULL AND externallogid =" + nPointID.ToString() + " Limit 1"
        Else
            sQuery = "SELECT Value FROM alldata WHERE timestamp > ? AND Value IS NOT NULL AND externallogid =" + nPointID.ToString() + " order by timestamp Limit 1"
        End If
    ...
    Private Sub CalcualteTopBottomBodyMarginsForTrendAlarm(ByRef nTopMargin As Integer, ByRef nBottomMargin As Integer, ByRef nReporttype As ReportType)
        'Dim nBodyDeaderYPos As Integer
        ...
        If nReporttype = ReportType.DataTrendAlarmReport Then
            Dim nAlarmColCount As Integer = 5
            ...
            Next
            oBodyHeaderTable.SetWidths(sColWidths)
            Dim sColumName As String() = {"Date Time", "Alarm Message", "Alarm Normalized time", "Acknowledge Data Time", "User Comments"}
            For nCol = 0 To 5 - 1
